...
### 对称视角：为什么偏 sdiff 侧反而稳定（钉死机制）

两侧其实**都有** `max` 反超，只是方向相反：偏 mdl 端 sdiff 项反超 → 拽向先验；偏 sdiff 端 mdl 项反超 → 推离先验。真正的不对称来自 sdiff 的几何特殊性——它有**离散硬下界、且贴先验的小值可达**，而 mdl 没有这样的离散可达最优点（`ideal_mdl` 移动、连续逼近、够不着）。

**偏 sdiff 子问题被"钉死"**。边际账（range_sdiff≈60）：离开钉死值（sdiff +1 一步）的代价 ≈ `λ_sdiff·(1/60)=0.99×0.0167≈0.0165`，超过弱 `λ_mdl` 能给的 mdl 收益（`0.01·Δd_mdl ≤ 0.01`）→ 离开永远亏 → 钉死。偏 mdl 子问题没有这样的钉死点，只能止步于次优 mdl——这就是牵引。

**实测坐实**（alarm+random+N1000，800 代，与上文机制诊断同案例）：

| 子问题 | λ_sdiff | sdiff 轨迹 | 后半程 std |
|---|:---:|---|:---:|
| i1 偏 sdiff | 0.99 | 3–5 代内降到 **2 并冻结** | **0**（零波动） |
| i0 纯 sdiff | 1.0 | 全 800 代恒 **2** | 0 |
| im2 偏 mdl | 0.01 | 停在 **58–65** 满量程 | 2.62（持续波动） |

- **不对称**：末代 i1=2 vs im2=65，**32.5 倍**。
- **权重全谱**：sdiff 随 λ_sdiff **强单调下降**（λ=0→59，0.5→27，0.9→7，0.99→2）；最偏 sdiff 的 10 个子问题 sdiff=[2,2,3,3,4,5,5,6,6,7]，全钉在 2–7。
- **修正**：钉死值不是恰好 0，而是该 prior+约束下**最小可达 sdiff**（此处 2，`n_eq0` 全程为 0）；机制实质不变。

**推广解释"整体贴先验"**：对任意 λ 子问题，离开先验代价 `λ_sdiff·(1/range)` vs 收益 `λ_mdl·Δd_mdl`。mild 先验已接近数据最优 → `Δd_mdl` 小 → 一大片子问题钉死 → 结果贴先验；random 先验离开收益大 → 子问题放行。这比单看"max 反超"更统一地解释了第二节"贴先验程度随先验质量递减"，以及老观察"sdiff 实际≤16"。
...
- 机制实测：alarm + random + N1000，临时打点 800 代，含偏 mdl/偏 sdiff 双侧子问题 sdiff 轨迹（诊断后源码已还原）
